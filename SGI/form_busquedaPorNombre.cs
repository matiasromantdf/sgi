    ...
    public partial class form_busquedaPorNombre : Form
    {
        ...
        private void trasladar()
        {
           if (origen == "ventas")
            ...
            if (origen == "bajas")
            {
                form_bajaArticulo padre = Owner as form_bajaArticulo;
                padre.txt_codigo.Text = data_grid_resultados.CurrentRow.Cells[0].Value.ToString();
                this.Close();
                padre.txt_codigo.Focus();
            }
            if (origen == "consulta")
            {
                form_consultarArticulo padre = Owner as form_consultarArticulo;
                padre.txt_codigo.Text = data_grid_resultados.CurrentRow.Cells[0].Value.ToString();
                this.Close();
                padre.txt_codigo.Focus();
            }
            if (origen == "editar")
            {
                //form_editarArticulo padre = Owner as form_editarArticulo;
                //padre.txt_codigo.Text = data_grid_resultados.CurrentRow.Cells[0].Value.ToString();
                //this.Close();
               // padre.txt_codigo.Focus();
            }
